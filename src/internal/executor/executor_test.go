...
func TestOverlayRequestEnvKeepsBaselineAndScrubsAuth(t *testing.T) {
	env := overlayRequestEnv(
		[]string{
			"HOME=/home/daemon",
			"PATH=/usr/bin",
			"GH_TOKEN=daemon-token",
			"GH_ENTERPRISE_TOKEN=daemon-enterprise-token",
			"GH_HOST=daemon.example.com",
		},
		[]string{"GH_TOKEN=client-token", "GH_ENTERPRISE_TOKEN=client-enterprise-token", "GH_REPO=owner/repo"},
	)
	...
	if strings.Contains(joined, "daemon-token") || strings.Contains(joined, "daemon-enterprise-token") || strings.Contains(joined, "daemon.example.com") {
		t.Fatalf("expected daemon auth/context env to be scrubbed, got %#v", env)
	}
	if !strings.Contains(joined, "GH_TOKEN=client-token") || !strings.Contains(joined, "GH_ENTERPRISE_TOKEN=client-enterprise-token") || !strings.Contains(joined, "GH_REPO=owner/repo") {
